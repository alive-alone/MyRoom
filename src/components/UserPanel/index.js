      ...
      <div className="w-3/4">
        ...
        <div className="">
          ...
          <div className="mb-3 h-10 flex items-baseline">
            <span className="text-base h-full flex-0 leading-loose">密码：</span>
            <input type="password" placeholder="please input password" className="h-full px-2 ml-4 w-full flex-1" />
          </div>
        </div>
        <div className="flex justify-evenly text-base">
          <button type="submit" className="bg-green-500 text-gray-600 rounded px-5 py-2" onClick={() => props.leftButtonEvent()}>{props.leftButtonText}</button>
          <button className="bg-zinc-300 text-gray-600 rounded px-5 py-2" onClick={() => props.rightButtonEvent()}>{props.rightButtonText}</button>
        </div>
      </div>
    </div>
  );
